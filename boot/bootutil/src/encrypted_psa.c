...
boot_decrypt_key(const uint8_t *buf, uint8_t *enckey)
{
    ...
    /* QQQ: There is used mixed set of defines for the same things that is very
     * confusing; for example derived key is divided into AES encryption key
     * and HMAC key for MAC tagging, which is represented by sizes
     * BOOTUTIL_CRYPTO_AES_CTR_KEY_SIZE and BOOTUTIL_CRYPTO_SHA256_DIGEST_SIZE
     * respectively giving a derived key of the sum of the above.
     * Yet, when accessing the later part of derived key, in the key buffer
     * containing both, we are using BOOT_ENC_KEY_SIZE; the last one is
     * the same as BOOTUTIL_CRYPTO_AES_CTR_KEY_SIZE.
     * And then we also use the EC_ indexes into buffer which again are just
     * redefines of the one or another size.
     */
